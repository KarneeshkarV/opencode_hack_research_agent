...
TEAM_INSTRUCTIONS = [
    ...
    "brokerage order → delegate IMMEDIATELY to execution-agent in ONE shot. Pass "
    "the user's literal phrasing plus any context you have (resolved ticker, prior "
    "LTP from a recent run, the user's holdings if known). Do NOT ask the user to "
    "pick exchange/product/order_type/AMO/quantity/confirmation — execution-agent "
    "has hard defaults (NSE, CNC, MARKET, market_protection=-1, qty=1, regular→amo "
    "auto-retry) and can fetch profile/LTP/holdings itself. Just route through and "
    "return its result verbatim. Never call place_kite_order directly. "
